var main = {
    ...
    commentSave : function () {
        ...
        if (!data.content || data.content.trim() === "") { // 공백 및 빈 문자열 체크
            ...
            $.ajax({
                type: 'POST',
                url: '/api/boards/detail/' + data.boardId,
                dataType: 'text',
    ...
    commentUpdate : function (form) {
        ...
        if (con_check === true) {
            $.ajax({
                type: 'PUT',
                url: '/api/boards/detail/' + data.boardId + '/comments/' + data.id,
                dataType: 'JSON',
                contentType: 'application/json; charset=utf-8',
                data: JSON.stringify(data)
            }).done(function () {
    ...
    commentDelete : function(boardId, commentId) {
        ...
        if (con_check === true) {
            $.ajax( {
                type: 'DELETE',
                url: '/api/boards/detail/' + boardId + '/comments/' + commentId,
                dataType: 'JSON',
            }).done(function () {
                alert('댓글이 삭제되었습니다.');
                window.location.reload();
